...
class leagueOfLegends():
	...
	async def summonerid(self, summoner):
		...
		params = {
			"api_key": private.lolApi
		}

		#pulls the summoners ID from his username
		leaguePull = requests.get("https://na.api.pvp.net/api/lol/na/v1.4/summoner/by-name/"+summoner+"?"+private.lolApi)
	...
	async def freechamps(self):
		...
		leaguePull = requests.get("https://na.api.pvp.net/api/lol/na/v1.2/champion?freeToPlay=true&"+private.lolApi)
		...
		while aNumber < 10:
			championName = requests.get("https://global.api.pvp.net/api/lol/static-data/na/v1.2/champion/"+str(leaguePull.json()["champions"][draven]["id"])+"?champData=info,recommended&"+private.lolApi)
	...
	async def matchhistory(self, summoner, region:str):
		...
		if region == "eu" or region == "euw":
			...
			leaguePull = requests.get("https://euw.api.pvp.net/api/lol/euw/v1.4/summoner/by-name/"+summoner+"?"+private.lolApi)
			if leaguePull.status_code == 200:
				summoner = str(leaguePull.json()[summoner]["id"])
			else:
				await self.bot.say(leaguePull.status_code)
			#pulling the game stats from the last game
			leaguePull = requests.get("https://euw.api.pvp.net/api/lol/euw/v1.3/game/by-summoner/"+summoner+"/recent?"+private.lolApi)
			if leaguePull.status_code == 200:
				...
				championPull = requests.get("https://global.api.pvp.net/api/lol/static-data/euw/v1.2/champion/"+str(leaguePull.json()["games"][0]["championId"])+"?champData=info,recommended&"+private.lolApi)
				...
				championPull = requests.get("https://global.api.pvp.net/api/lol/static-data/euw/v1.2/champion/"+str(leaguePull.json()["games"][1]["championId"])+"?champData=info,recommended&"+private.lolApi)
				championUsed = str(championPull.json()["name"])
			...
			if leaguePull.status_code == 200:
				...
				await self.bot.say(leaguePull.status_code)
			#pulling the game stats from the last game
			leaguePull = requests.get("https://na.api.pvp.net/api/lol/na/v1.3/game/by-summoner/"+summoner+"/recent?"+private.lolApi)
			if leaguePull.status_code == 200:
				if leaguePull.json()["games"][0]["stats"]["win"] == True:
					winStatus = "****Game Won!****"
				else:
				...
				gameLength = (int(leaguePull.json()["games"][0]["stats"]["timePlayed"]) / 60)
				#pulls champipn name from it's ID
				championPull = requests.get("https://global.api.pvp.net/api/lol/static-data/na/v1.2/champion/"+str(leaguePull.json()["games"][0]["championId"])+"?champData=info,recommended&"+private.lolApi)
				championUsed = str(championPull.json()["name"])
		...
		if region == "na":
			...
			#finding the ID from the username
				...
				if leaguePull.status_code == 200:
					...
					await self.bot.say(leaguePull.status_code)
				#pulling the stats
				leaguePull = requests.get("https://na.api.pvp.net/api/lol/na/v1.3/stats/by-summoner/"+summoner+"/summary?season=SEASON2015&"+private.lolApi)
				aNumber = 0
				unranked = []
				rankedSolo = []
				rankedTeam = []
			...
			rankedCheck = requests.get("https://na.api.pvp.net/api/lol/na/v1.3/stats/by-summoner/"+summoner+"/ranked?season=SEASON2015&"+private.lolApi)
			...
			try:
				...
				#finding the ID from the username
				leaguePull = requests.get("https://"+summonerRegion+".api.pvp.net/api/lol/"+summonerRegion+"/v1.4/summoner/by-name/"+summoner+"?"+private.lolApi)
				if leaguePull.status_code == 200:
					summoner = str(leaguePull.json()[summoner]["id"])
				else:
					await bot.say(leaguePull.status_code)
			#pulling the stats
			leaguePull = requests.get("https://"+summonerRegion+".api.pvp.net/api/lol/"+summonerRegion+"/v1.3/stats/by-summoner/"+summoner+"/summary?season=SEASON2015&"+private.lolApi)
			aNumber = 0
			unranked = []
			rankedSolo = []
			rankedTeam = []
			...
			rankedCheck = requests.get("https://"+summonerRegion+".api.pvp.net/api/lol/"+summonerRegion+"/v1.3/stats/by-summoner/"+summoner+"/ranked?season=SEASON2015&"+private.lolApi)
			if rankedCheck.status_code == 200:
				aNumber = 0
				while str(leaguePull.json()["playerStatSummaries"][aNumber]["playerStatSummaryType"]) != "RankedSolo5x5":
					aNumber += 1
